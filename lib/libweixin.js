...
var lists_web_path = function(key, entry) {
  return prepend_url('/lists/' + encodeURI(key) + (entry ? '/' + encodeURI(entry) : ''));
};
...
var make_robot_list = function(key) {
  if (config.lists && config.lists.hasOwnProperty(key)) {
    ...
    for (var i = 0; i < Math.min(9, items.length); i++) {
      ...
      if (links.length > 0) {
        links = link_base + links;
      } else {
        links = lists_web_path(key, items[i].title);
      }
