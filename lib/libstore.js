...
var find_stores = function(stores_list, store_name) {
  var stores = [];
  for(var i = 0; i < stores_list.length; i++) {
    if (stores_list[i][0] == store_name) stores.push(stores_list[i]);
  }
  return stores;
};
...
var find_nearby_stores = function(stores_list, latitude, longitude) {
  ...
  if (/^\-?([0-9]|[1-8][0-9]|90)\.[0-9]{1,6}$/.test(latitude)) {
    if (/^\-?([0-9]{1,2}|1[0-7][0-9]|180)\.[0-9]{1,6}$/.test(longitude)) {
      ...
      assert.strictEqual(stores_list[0].length, 12, 'module cache problem?');
    }
  }
  return stores_list;
};
...
var baidu_maps_static_img_url = function(x, y) {
  ...
  url += '?width=640&height=320&center=%s&zoom=16&markers=%s&markerStyles=m';
  var coord = y + ',' + x;
...
exports.find_nearby_stores_to_weixin_list = function(stores_list, latitude, longitude) {
  ...
  var weixin_list = [];
  weixin_list.push([config.title_of_first_nearby_store, '',
                    baidu_maps_static_img_url(list[0][9], list[0][10]),
                    config.base_url + '/maps?lat=' + latitude + '&lng=' + longitude ]);

  for (var i = 0; i < Math.min(config.number_of_nearby_stores_to_list, list.length); i++) {
    var title = list[i][0] + '\n' + 
        '地址：' + list[i][1] + (list[i][1] != list[i][2] ? list[i][2] : '') +
                  list[i][3] + list[i][4] + '\n' +
        '直线距离：' + list[i][11] + 'km';
    weixin_list.push([ title, '',
                       config.default_nearby_store_image_url,
                       config.base_url + '/stores/' + encodeURI(list[i][0]) +
