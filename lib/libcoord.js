/*
 * Get coordinates using Baidu Maps.
 */
...
var convert_point_to_coordinates = function(x, y) {
  ...
  for (var i = 0; i < MCBAND.length; i++) {
    if (y >= MCBAND[i]) {
      ...
      var INT = y / E[9];
      var LAT = E[2] + E[3] * Math.pow(INT, 1) + E[4] * Math.pow(INT, 2)
                     + E[5] * Math.pow(INT, 3) + E[6] * Math.pow(INT, 4)
                     + E[7] * Math.pow(INT, 5) + E[8] * Math.pow(INT, 6);
      LNG *= (x < 0 ? -1 : 1);
      LAT *= (y < 0 ? -1 : 1);
...
exports.distance_between_coordinates = function(lat1, lng1, lat2, lng2) {
  var earth_radius = 6371; // km
  var delta_lat = (lat2 - lat1).toRad();
  var delta_lng = (lng2 - lng1).toRad();
  lat1 = lat1.toRad();
  lat2 = lat2.toRad();
  var square_of_half_chord_length = Math.sin(delta_lat / 2) * Math.sin(delta_lat / 2) +
                                    Math.sin(delta_lng / 2) * Math.sin(delta_lng / 2) *
                                    Math.cos(lat1) * Math.cos(lat2);
  var angular_distance = 2 * Math.atan2(Math.sqrt(square_of_half_chord_length),
                                        Math.sqrt(1 - square_of_half_chord_length));
